...
import (
	...
	"github.com/rocketseat-eduaction/gobid/internal/store/pgstore"
...
func (us *UserService) CreateUser(ctx context.Context, userName, email, password, bio string) (uuid.UUID, error) {
	...
	user, err := us.queries.CreateUser(ctx, args) // user = pgstore.User
	if err != nil {
	...
	return user.ID, nil // devolvemos solo el campo ID (que sí es uuid.UUID)
}
